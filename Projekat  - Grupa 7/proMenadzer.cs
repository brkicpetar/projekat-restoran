...
", item);
            ...
            if (novaCena == "") novaCena = File.ReadAllLines(Path.GetDirectoryName(jela[index]) + @"\" + item + @"\o_jelu.pf")[1];
            ...
            else
            {
                ...
                Console.CursorVisible = false;
                if (noviSastojciJela == "") noviSastojciJela = File.ReadAllLines(Path.GetDirectoryName(jela[index]) + @"\" + item + @"\o_jelu.pf")[0];
            }

            Console.CursorVisible = false;
            File.WriteAllText(Path.GetDirectoryName(jela[index]) + @"\" + item + @"\o_jelu.pf", noviSastojciJela + Environment.NewLine + novaCena);
            Console.WriteLine(Environment.NewLine + "Izmene uspešno unete!");
            System.Threading.Thread.Sleep(1500);
            IzmenaJelovnika();
        }
        ...
        private void IzmenaKartePica()
        {
            ...
            Console.Clear();
            Console.ResetColor();
            proMenu meniIzmenaKartePica = new proMenu(new string[] { "Novo piće", "Izmeni piće", "Briši piće", "Dodaj novu kategoriju", "Izmeni kategoriju", "Briši kategoriju", "Povratak na menadžerski meni" }, @"                           _ _ _              _ _                    _____           _                        
──────▄▀─      /\         | (_) |            (_|_)                  |  __ \         | |                       
─█▀▀▀█▀█─     /  \   _ __ | |_| | ____ _  ___ _ _  __ _   ______ _  | |__) |___  ___| |_ ___  _ __ __ _ _ __  
──▀▄░▄▀──    / /\ \ | '_ \| | | |/ / _` |/ __| | |/ _` | |_  / _` | |  _  // _ \/ __| __/ _ \| '__/ _` | '_ \ 
────█────   / ____ \| |_) | | |   < (_| | (__| | | (_| |  / / (_| | | | \ \  __/\__ \ || (_) | | | (_| | | | |
...
Izmena karte pića:
");
            List<string> kategorijeSaPathovima = Directory.GetDirectories(Properties.Resources.LokacijaPomocnihFajlova + @"Karta pica\").ToList();
            List<string> kategorije = new List<string>();
            List<string> kategorijeZaMeni = new List<string>();
            foreach (var item in kategorijeSaPathovima)
            {
            ...
            int izabraniIndex = meniIzmenaKartePica.PokreniMeni();
            if (izabraniIndex == 6)
            {
                Console.Clear();
                Init();
...
Unesite ime nove kategorije:");
                ...
                Console.CursorVisible = false;
                string[] dirs = Directory.GetDirectories(Properties.Resources.LokacijaPomocnihFajlova + @"Karta pica\");
                string broj = (int.Parse(Path.GetFileName(dirs[dirs.Length - 1].Split('.')[0].Trim())) + 1).ToString();
                Directory.CreateDirectory(Properties.Resources.LokacijaPomocnihFajlova + @"Karta pica\" + broj + ". " + imeKategorije);
                IzmenaKartePica();
            }
            else if (izabraniIndex == 4)
            {
...
Novo ime za kategoriju {0}: ", kategorije[index]);
                    ...
                    Directory.Move(kategorijeSaPathovima[index], Properties.Resources.LokacijaPomocnihFajlova + @"Karta pica\" + broj + ". " + novoIme);
                    Console.WriteLine(Environment.NewLine + "Izmene uspešno unete!");
                    System.Threading.Thread.Sleep(1500);
                    IzmenaKartePica();
                }
...
");
                ...
                {
                    List<string> picaSaPathovima = Directory.GetDirectories(Properties.Resources.LokacijaPomocnihFajlova + @"Karta pica\" + kategorije[izabranaKategorija] + @"\").ToList();
                    List<string> pica = new List<string>();
                    foreach (var item in picaSaPathovima) pica.Add(Path.GetFileName(item).Trim());
                    pica.Add("Povratak na izbor kategorija");
                    proMenu izborPica = new proMenu(pica.ToArray(), @"                           _ _ _              _ _                    _____           _                        
──────▄▀─      /\         | (_) |            (_|_)                  |  __ \         | |                       
─█▀▀▀█▀█─     /  \   _ __ | |_| | ____ _  ___ _ _  __ _   ______ _  | |__) |___  ___| |_ ___  _ __ __ _ _ __  
──▀▄░▄▀──    / /\ \ | '_ \| | | |/ / _` |/ __| | |/ _` | |_  / _` | |  _  // _ \/ __| __/ _ \| '__/ _` | '_ \ 
────█────   / ____ \| |_) | | |   < (_| | (__| | | (_| |  / / (_| | | | \ \  __/\__ \ || (_) | | | (_| | | | |
...
Izaberite piće koje želite da obrišete iz karte pića:
");
                    int izabranoPice = izborPica.PokreniMeni();
                    if (izabranoPice == picaSaPathovima.Count) goto Line72;
                    else
                    {
                        Directory.Delete(picaSaPathovima[izabranoPice], true);
                        Console.WriteLine(Environment.NewLine + "Brisanje uspešno");
                        System.Threading.Thread.Sleep(1500);
                        IzmenaKartePica();
                    }
...
Izaberite kategoriju u koju želite da dodate piće:
");
                int izabranaKategorija = izborKategorije.PokreniMeni();
                string kat = "";
                if (izabranaKategorija == kategorije.Count)
...
");
                ...
                while (imeNovogPica == "")
                {
                    Console.Write("Podaci nisu uneti! Molim unesite ime novog pića: ");
                    imeNovogPica = Console.ReadLine();
                }
                Console.Write("Cena u dinarima (bez PDV-a): ");
                string cena = Console.ReadLine();
                while (!int.TryParse(cena, out int tempInt) || tempInt == 0)
                ...
                Console.Write("Proizvođač (nije obavezno): ");
                string detaljiOPicu = Console.ReadLine();

                detaljiOPicu = detaljiOPicu == "" ? "<==>" : detaljiOPicu;

                Console.CursorVisible = false;

                Directory.CreateDirectory(Properties.Resources.LokacijaPomocnihFajlova + @"Karta pica\" + kat + @"\" + imeNovogPica);
                File.WriteAllText(Properties.Resources.LokacijaPomocnihFajlova + @"Karta pica\" + kat + @"\" + imeNovogPica + @"\o_jelu.pf", detaljiOPicu + Environment.NewLine + cena, Encoding.Unicode);
                Console.WriteLine(Environment.NewLine + "Novo piće uspešno dodato!");
                System.Threading.Thread.Sleep(1500);
                IzmenaKartePica();
            }
            else if (izabraniIndex == 1)
...
");
                ...
                List<string> picaSaPathovima = Directory.GetDirectories(Properties.Resources.LokacijaPomocnihFajlova + @"Karta pica\" + kat + @"\").ToList();
                List<string> pica = new List<string>();
                foreach (var item in picaSaPathovima) pica.Add(Path.GetFileName(item).Trim());
                pica.Add("Povratak na izbor kategorija");
                proMenu izborPica = new proMenu(pica.ToArray(), @"                           _ _ _              _ _                    _____           _                        
...
");
                int izabranoPice = izborPica.PokreniMeni();
                string piceZaIzmenu = "";
                if (izabranoPice == picaSaPathovima.Count) goto Line189;
                else piceZaIzmenu = pica[izabranoPice];

                IzmenaPica(izabranoPice, piceZaIzmenu, picaSaPathovima.ToArray());
            }
        }
...
", item);
            ...
            if (novoIme != "")
            {
                Directory.Move(pica[index], Path.GetDirectoryName(pica[index]) + @"\" + novoIme);
                item = novoIme;
            }
            Console.Write("Nova cena pića (ukoliko ne želite da promenite cenu, ostavite prazno): ");
            string novaCena = Console.ReadLine();
            if (novaCena == "") novaCena = File.ReadAllLines(Path.GetDirectoryName(pica[index]) + @"\" + item + @"\o_jelu.pf")[1];
            Console.Write("Da li želite da obrišete proizvođača pića (da/ne): ");
            string brisati = Console.ReadLine().Trim();
            string novProizvodjac = "";
            while (brisati != "da" && brisati != "ne")
            {
                Console.WriteLine("Nije dobar unos! Pokušajte ponovo: ");
                brisati = Console.ReadLine();
            }
            if (brisati == "da") novProizvodjac = "<==>";
            else
            {
                Console.Write("Nov proizvođač pića (ukoliko ne želite da promenite proizvođača, ostavite prazno): ");
                novProizvodjac = Console.ReadLine();
                Console.CursorVisible = false;
                if (novProizvodjac == "") novProizvodjac = File.ReadAllLines(Path.GetDirectoryName(pica[index]) + @"\" + item + @"\o_jelu.pf")[0];
            }
            Console.CursorVisible = false;
            File.WriteAllText(Path.GetDirectoryName(pica[index]) + @"\" + item + @"\o_jelu.pf", novProizvodjac + Environment.NewLine + novaCena);
            Console.WriteLine(Environment.NewLine + "Izmene uspešno unete!");
            System.Threading.Thread.Sleep(1500);
            IzmenaKartePica();
        }
